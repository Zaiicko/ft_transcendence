...
import ScreenshotGuessLocalPlay from './ScreenshotGuessLocalPlay';

const DIFFICULTIES: ScreenshotGuessDifficulty[] = ['easy', 'normal', 'hard'];
const ROUND_MODES: ScreenshotGuessRoundMode[] = ['TURNS', 'RACE'];
...
// for a snappier race.
const ANSWER_TIMES: Record<ScreenshotGuessRoundMode, number[]> = {
  TURNS: [10, 15, 30],
  RACE: [5, 10, 15, 30],
};
...
export default function ScreenshotGuessSetup() {
  ...
  async function createMultiMatch() {
    ...
    setError(null);
    try {
      const { matchId } = await apiFetch<{ matchId: string }>('/minigames/screenshot-guess/matches', {
        method: 'POST',
        body: JSON.stringify({
          difficulty,
          roundMode,
      ...
      navigate(`/minigames/screenshot-guess/match/${matchId}`);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-center gap-4">
        <ScreenshotGuessLogo className="aspect-[3/4] w-16 shrink-0" />
        <SectionHead
          className="mb-0"
          eyebrow={t('minigames.hub.eyebrow')}
          title={t('minigames.screenshotGuess.title')}
        />
      </div>
      <p className="text-sm text-zinc-500 dark:text-zinc-400">{t('minigames.screenshotGuess.description')}</p>

      <div className="card flex flex-col gap-6 p-5">
        <div>
          <p className="mb-2 text-sm font-semibold">{t('minigames.screenshotGuess.setup.mode')}</p>
          <div className="flex gap-2">
            ...
            >
              {t('minigames.screenshotGuess.setup.modeLocal')}
            </button>
            <button
              type="button"
            ...
            >
              {t('minigames.screenshotGuess.setup.modeMulti')}
            </button>
          </div>
          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
            {t(`minigames.screenshotGuess.setup.modeHint.${mode}`)}
          </p>
        </div>

        <div>
          <p className="mb-2 text-sm font-semibold">{t('minigames.screenshotGuess.setup.difficultyLabel')}</p>
          <div className="flex gap-2">
            {DIFFICULTIES.map((d) => (
              <button
                key={d}
              ...
              >
                {t(`minigames.screenshotGuess.setup.difficulty.${d}`)}
              </button>
            ))}
          </div>
          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
            {t(`minigames.screenshotGuess.setup.difficultyHint.${difficulty}`)}
          </p>
        </div>

        <div>
          <p className="mb-2 text-sm font-semibold">{t('minigames.screenshotGuess.setup.roundMode')}</p>
          <div className="flex gap-2">
            {ROUND_MODES.map((m) => (
              <button
                key={m}
              ...
              >
                {t(`minigames.screenshotGuess.setup.roundModeLabel.${m}`)}
              </button>
            ))}
          </div>
          <p className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
        ...
        <div>
          <p className="mb-2 text-sm font-semibold">{t('minigames.screenshotGuess.setup.targetScore')}</p>
          <div className="flex gap-2">
            {TARGET_SCORES.map((s) => (
              <button
                key={s}
        ...
        {mode === 'local' ? (
          <div>
            <p className="mb-2 text-sm font-semibold">{t('minigames.screenshotGuess.setup.localPlayers')}</p>
            <div className="flex items-center gap-3">
              <button
                type="button"
              ...
              <div className="mt-3 flex flex-col gap-2">
                {Array.from({ length: localPlayerCount - 1 }).map((_, i) => (
                  <input
                    ...
                    placeholder={t('minigames.screenshotGuess.setup.guestNamePlaceholder', { n: i + 2 })}
                    maxLength={24}
          ...
          <div>
            <p className="mb-2 text-sm font-semibold">{t('minigames.screenshotGuess.setup.inviteFriends')}</p>
            {friends.length === 0 ? (
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                {t('minigames.screenshotGuess.setup.noFriendsOnline')}
              </p>
            ) : (
            ...
            >
              {t('minigames.screenshotGuess.setup.sendInvites')}
            </button>
          </div>
        )}
      </div>
