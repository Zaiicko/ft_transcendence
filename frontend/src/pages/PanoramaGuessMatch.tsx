...
import { usePanoramaGuessSocket } from '../minigames/usePanoramaGuessSocket';
import type { PanoramaGuessMatchState } from '../minigames/types';

export default function PanoramaGuessMatch() {
  const { matchId } = useParams<{ matchId: string }>();
  const { user } = useAuth();
  const { t } = useTranslation();
  const navigate = useNavigate();

  const [state, setState] = useState<PanoramaGuessMatchState | null>(null);
  ...
  const load = useCallback(() => {
    if (!matchId) return;
    apiFetch<PanoramaGuessMatchState>(`/minigames/panorama-guess/matches/${matchId}`)
      .then(setState)
      .catch((err) =>
        setError(err instanceof ApiError ? err.message : t('minigames.panoramaGuess.errors.matchNotFound')),
      )
  ...
  usePanoramaGuessSocket(!!user && !!matchId, (s) => {
    if (s.id === matchId) setState(s);
  });

  // Purely cosmetic countdown — the server enforces the actual round timeout
  // and will push a fresh state once it fires.
  useEffect(() => {
  ...
  async function respond(accept: boolean) {
    ...
    setError(null);
    try {
      const next = await apiFetch<PanoramaGuessMatchState>(`/minigames/panorama-guess/matches/${matchId}/respond`, {
        method: 'POST',
  ...
  async function start() {
    ...
    try {
      setState(
        await apiFetch<PanoramaGuessMatchState>(`/minigames/panorama-guess/matches/${matchId}/start`, {
          method: 'POST',
        }),
      );
    } catch (err) {
  ...
  async function guess(catalogId: number) {
    ...
    try {
      setState(
        await apiFetch<PanoramaGuessMatchState>(`/minigames/panorama-guess/matches/${matchId}/guess`, {
          method: 'POST',
  ...
  async function leave() {
    if (!matchId) return;
    try {
      await apiFetch(`/minigames/panorama-guess/matches/${matchId}/leave`, { method: 'POST' });
    } catch {
      // best-effort
    }
    navigate('/minigames');
  ...
  if (loading) return <p className="text-sm text-zinc-500">{t('minigames.panoramaGuess.errors.loading')}</p>;
    ...
    <div className="flex flex-col gap-8">
      ...
      {state.status === 'ABANDONED' && (
        <div className="card p-6 text-center text-sm text-zinc-500 dark:text-zinc-400">
          {t('minigames.panoramaGuess.errors.matchEnded')}
        </div>
      )}
        ...
        <div className="card flex flex-col gap-4 p-5">
          <ul className="flex flex-col gap-2">
            {state.players.map((p) => (
              <li key={p.userId} className="flex items-center gap-2.5 text-sm">
                ...
                >
                  {p.status === 'ACCEPTED'
                    ? t('minigames.panoramaGuess.invite.accepted')
                    : p.status === 'DECLINED'
                      ? t('minigames.panoramaGuess.invite.declined')
                      : t('minigames.panoramaGuess.invite.pending')}
                </span>
              </li>
            ))}
          </ul>
          ...
          {me?.status === 'PENDING' ? (
            <div className="flex gap-3">
              ...
              >
                {t('minigames.panoramaGuess.invite.accept')}
              </button>
              <button
                type="button"
                disabled={busy}
              ...
              >
                {t('minigames.panoramaGuess.invite.decline')}
              </button>
            </div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              {t('minigames.panoramaGuess.invite.waitingForHost')}
            </p>
          )}
        </div>
      )}
          ...
          <div className="flex flex-wrap gap-3 text-sm">
            {state.players
              ...
              .map((p) => (
                <span
                  key={p.userId}
                  className="flex items-center gap-2 rounded-full bg-zinc-100 px-3 py-1 font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300"
                >
            ...
            <span className="ml-auto self-center text-zinc-400">
              {t('minigames.panoramaGuess.play.targetScore', { count: state.targetScore })}
            </span>
          </div>

          <div className="card flex flex-col items-center gap-4 p-6">
            <PanoramaEmbed kuulaId={state.round.kuulaId} title={t('minigames.panoramaGuess.play.embedTitle')} />

            {state.round.resolved ? (
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                {t('minigames.panoramaGuess.play.revealedAnswer', { title: state.round.answerTitle })}
              </p>
            ) : (
              <>
                <p className="text-sm font-medium">
                  {t('minigames.panoramaGuess.play.racePromptOnline')}
                  {remaining !== null && (
      ...
      {state.status === 'FINISHED' && (
        <div className="card flex flex-col items-center gap-4 p-8 text-center">
          <span className="font-display text-2xl font-bold">{t('minigames.panoramaGuess.match.finished')}</span>
                ...
                return (
                  <li
                    ...
                    className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm ${
                      p.userId === state.winnerId ? 'bg-accent/15' : 'bg-zinc-100/70 dark:bg-zinc-800/70'
                    }`}
                  >
      ...
      {(state.status === 'LOBBY' || state.status === 'PLAYING') && (
        <button type="button" onClick={() => void leave()} className="self-start text-sm text-zinc-500 hover:text-accent">
          {t('minigames.panoramaGuess.match.leave')}
        </button>
      )}
